...
export const PictureMissionScreen: React.FC<PictureMissionScreenProps> = ({
  ...
  const handleAnswer = (chosen: BodyPart) => {
    ...
    if (chosen.id === targetPart.id) {
      ...
      playSuccessChime();
      playStarEarned();
      speakEnglish(`Awesome! That is ${targetPart.word}!`);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center text-center pt-1 z-10">
        <div className="flex items-center gap-3 bg-white px-6 py-2.5 rounded-2xl border-3 border-indigo-300 shadow-md">
          <h2 className="text-3xl md:text-5xl font-black text-indigo-900 tracking-tight">
            {questionSentence}
          </h2>
          <button
            type="button"
            onClick={handleSpeakQuestion}
            className="p-3 bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white rounded-xl shadow-md transition-all shrink-0"
        ...
        <p className="text-sm md:text-base font-extrabold text-slate-600 mt-2 flex items-center gap-2">
          <span>몬스터를 마우스로 끌어서 알맞은 그림에 닿게 해주세요! 🎨</span>
          <span className="text-indigo-600 font-black">({targetPart.korean})</span>
        </p>
      </div>

      {/* Middle Section: 3 Picture Choice Cards (Drop Targets) */}
      <div className="my-auto py-2 z-10 w-full">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 max-w-3xl mx-auto w-full">
          {options.map((option) => {
            ...
            let cardStyles = 'bg-white border-slate-200 shadow-md hover:border-indigo-300';
            if (isHovered && !selectedId) {
              cardStyles = 'bg-indigo-50 border-indigo-500 ring-4 ring-indigo-300 scale-105 shadow-xl';
            } else if (isSelected) {
              if (feedback === 'correct') {
                cardStyles = 'bg-emerald-50 border-emerald-500 ring-4 ring-emerald-300 scale-105 shadow-2xl';
              } else if (feedback === 'wrong') {
                cardStyles = 'bg-rose-50 border-rose-500 ring-4 ring-rose-300 animate-shake';
              }
            }
            ...
            return (
              <div
                ...
                className={`p-6 md:p-8 rounded-3xl border-4 transition-all flex flex-col items-center justify-center gap-3 text-center cursor-pointer relative ${cardStyles}`}
              >
                {/* Visual Icon */}
                <div className="w-24 h-24 md:w-32 md:h-32 flex items-center justify-center p-2 rounded-2xl bg-slate-50">
                  <BodyPartIcon id={option.id} size={96} />
                </div>
                ...
                {isHovered && (
                  <div className="absolute -top-3 bg-indigo-600 text-white text-xs px-3 py-1 rounded-full font-black animate-bounce shadow-md">
                    몬스터를 여기에 퐁당! ✨
                  </div>
                )}
              </div>
      ...
      <div className="w-full flex flex-col items-center justify-center pb-2 z-20">
        ...
        <div className="mb-2 flex items-center gap-1.5 text-xs sm:text-sm font-black text-indigo-700 bg-indigo-50/90 px-3.5 py-1 rounded-full border border-indigo-200 shadow-2xs select-none">
          <Sparkles className="w-4 h-4 text-indigo-600 fill-indigo-300 shrink-0" />
          <span>몬스터를 꾹 잡아서 알맞은 그림 카드로 날려보세요!</span>
        </div>

        {/* Monster Rest Nest / Launch Base */}
        <div className="relative w-22 h-22 md:w-26 md:h-26 rounded-3xl border-3 border-dashed border-indigo-300 bg-indigo-100/40 flex items-center justify-center">
          {/* Ghost indicator when monster is dragged away */}
          {monsterPos && (
            <div className="flex flex-col items-center justify-center text-indigo-400 select-none pointer-events-none animate-pulse">
          ...
          >
            ...
            {isDragging && (
              <span className="absolute -bottom-7 bg-indigo-900/90 text-white text-[11px] font-black px-2.5 py-0.5 rounded-full whitespace-nowrap shadow-md pointer-events-none animate-pulse">
                정답 카드로 퐁당! ✨
              </span>
            )}
          </div>
